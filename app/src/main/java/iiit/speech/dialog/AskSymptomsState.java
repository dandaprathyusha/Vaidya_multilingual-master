    ...
    public void onRecognize(String hyp) {
        ...
        if (expect_binary && current_symptom != null) {
            ...
            if (nlu.resolveBinaryHyp(hyp)) {
                ...
                if(((HealthDomain) domain).get_cough_sample_confirmation(current_symptom)){
                    app.speakOut(app.getString(R.string.cough_sample_request), null);

                  /*  if (app.coughSampleRecord){
                        app.speakOut(app.getString(R.string.cough_sample_request_2), null);
                        app.coughSampleRecord=Boolean.TRUE;
                    }*/

                    if (app.coughSampleRecord) {

                        temp_cough_sym = current_symptom;
                        ((HealthDomain) domain).removeSymptom(temp_cough_sym);
                 //       current_symptom = ((HealthDomain) domain).SYMPTOM_CID.get(app.getCoughResult());
                        current_symptom = 44;
                        ((HealthDomain) domain).addSymptoms(current_symptom);
                    }
                }
                if( ((HealthDomain) domain).SYMPTOM_CID.get("wet cough") != current_symptom ||  ((HealthDomain) domain).SYMPTOM_CID.get("dry cough") != current_symptom){
